...
const PresentationEditor: React.FC<PresentationEditorProps> = ({
  ...
  const addShapeObject = (shape: 'rectangle' | 'ellipse') => addObject({ type: 'shape', x: 150, y: 150, width: 150, height: 100, animation: { type: 'fade-in' }, content: { shape, color: '#93c5fd' } });
  ...
  // Bug Fix: Base toolbar visibility on `selectedObjects`, not `selectedObjectIds`,
  // to prevent crashes when selection state is stale after an undo/redo.
  ...
  // Show contextual toolbar for slide properties when nothing is selected
  const showContextualToolbar = selectedObjects.length > 0 || selectedObjectIds.length === 0;
  ...
  return (
    <div className="flex h-full bg-gray-200 dark:bg-gray-900 text-gray-900 dark:text-gray-100">
      ...
      <div className="flex-1 flex flex-col" ref={editorRef}>
        <div className="bg-white dark:bg-gray-800 p-2 border-b border-gray-300 dark:border-gray-700">
